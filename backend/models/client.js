...
const inputModel = require('./input.js');
...
async function all() {
    try {
        ...
        const [rows, fieldDefs] = await db.pool.query(`
            SELECT ${tableName}.id, ${tableName}.name, input.code FROM ${tableName} 
            LEFT JOIN input ON ${tableName}.id = input.id
        `);
        var list = [];
        for (let row of rows) {
...
async function findByDept(code) {
    try {
        const [rows, fieldDefs] = await db.pool.query(`
        SELECT ${tableName}.id, ${tableName}.name, input.code FROM ${tableName} 
        INNER JOIN input ON ${tableName}.id = input.id AND input.code = ?`, [code]
        );
        var list = [];
        for (let row of rows) {
            let client = new Client(row.id, row.name, row.code);
...
async function insertOne(client) {
    try {
        ...
        if (exists.length == 0) {
            ...
            `, [client.name]);
            const clients_with_id = await findOneByName(client.name);
            if (clients_with_id.length > 0) {
                console.log(clients_with_id[0].id, client.code)
                const input = new inputModel.Input(clients_with_id[0].id, client.code);
                await inputModel.insertOne(work);    
            }
        }
    } catch (error) {
        console.error("database connection failed. " + error);
