...
function TrainerAvailability(props) {
	...
	// Retrieve passed data of workshop using props
  	const workshop = props.workshop;
	const assignedTrainerIds = workshop.assignedTrainers;

	// Retrieve all trainers from DB
	async function getAllTrainersFromDataBase() {
		const response = await fetch(`http://${back_url}/admin/trainers`);
		const data = await response.json();
		return data;
	}

	async function getParticularWorkshopFromDB(id) {

	}

	
	const [allTrainers, setAllTrainers] = useState({});
	const [assignedTrainers, setAssignTrainers] = useState({});

	// Get all trainers, as well as particular workshop data
	useEffect(() => {
		async function fetchTrainers() {
			const trainers = await getAllTrainersFromDataBase();
			setAllTrainers(trainers);
		};
		fetchTrainers();
		console.log()
	}, [allTrainers, workshop]);
