...
import { ClientNavbar } from '../src/components/Client_Navbar';
...
describe('ClientNavbar', () => {
    test('default selected tab is workshop request form', async () => {
        render(
          <BrowserRouter>
        ...
        const formTab = await screen.findByTestId('workshop request form-button');
        // Verify if the Home tab is rendered by checking if it's in the document
        expect(formTab).toBeInTheDocument();
    });

    test('selects the view workshops tab when clicked', async () => {
        render(
          <BrowserRouter>
              <ThemeProvider theme={theme}>
                  <ClientNavbar />
        ...
        // Wait for the state to be updated
        const viewTab = await screen.findByTestId('view workshops-button');
        // Verify if the Home tab is rendered by checking if it's in the document
